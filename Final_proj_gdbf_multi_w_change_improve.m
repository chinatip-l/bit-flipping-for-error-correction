...
for theta=0:-0.1:-1.5
    ...
    wbf_ber_multi_escape= zeros(size(EbN0dB,2),size(I_lim,2));
    wbf_ber_multi_escape(:,:)=-1;
    wbf_bler_multi_escape= zeros(size(EbN0dB,2),size(I_lim,2));
    wbf_bler_multi_escape(:,:)=-1;
    total_codeword_num_multi_escape = zeros(size(EbN0dB));
    ...
    for idx=1:length(EbN0dB)
        while any(wbf_bler_multi_escape(idx,:) < minimum_codeword_error_num)
            total_codeword_num_multi_escape(idx) = total_codeword_num_multi_escape(idx) + 1;
                ...
                if sum(Syn_wbf)>0
                    ...
                    if isempty(col)
                        ...
                        if wbf_Num_error_bit_temp > 0
                            wbf_ber_multi_escape(idx,col) = wbf_ber_multi_escape(idx,col) + wbf_Num_error_bit_temp
                            wbf_bler_multi_escape(idx,col) = wbf_bler_multi_escape(idx,col) + 1
                        end
                    end
                else
    ...
    WBF_BER_sim = wbf_ber_multi_escape./repmat((total_codeword_num_multi_escape*N)',1,size(I_lim,2))
    WBF_FER_sim = wbf_bler_multi_escape./repmat((total_codeword_num_multi_escape)',1,size(I_lim,2)) % FER is aka BLER
    ...
    name=sprintf('gdbf_multi_escape_improve_N%dK%d_theta_%.2f.mat',N,K,theta)
    ...
    hold off;
    legend;
    title(['Error-Rate Performance of (' num2str(N) ',' num2str(K) ') GDBF Multiple bit with escape Code; AWGN Channel']);
    xlabel('Eb/N0 (dB)');
    ylabel('Error Rates');

...
for theta1=0:-0.1:-1.5
    ...
    wbf_ber_multi_escape_p= zeros(size(EbN0dB,2),size(I_lim,2));
    wbf_ber_multi_escape_p(:,:)=-1;
    wbf_bler_multi_escape_p= zeros(size(EbN0dB,2),size(I_lim,2));
    wbf_bler_multi_escape_p(:,:)=-1;
    ...
    for idx=1:length(EbN0dB)
        while any(wbf_bler_multi_escape_p(idx,:) < minimum_codeword_error_num)
            ...
            for i=1:I_max
                ...
                if sum(Syn_wbf)>0
                    ...
                    if mode_flag == 0
                        ...
                        % check if switch back from single mode, if yes, then
                        % set to thena normal after jump
                        if theta~=theta1
                            theta=theta1
                        end
                    ...
                    if isempty(col)
                        ...
                        if wbf_Num_error_bit_temp > 0
                            wbf_ber_multi_escape_p(idx,col) = wbf_ber_multi_escape_p(idx,col) + wbf_Num_error_bit_temp;
                            wbf_bler_multi_escape_p(idx,col) = wbf_bler_multi_escape_p(idx,col) + 1
                        end
                    end
                else
    ...
    WBF_BER_sim = wbf_ber_multi_escape_p./repmat((total_codeword_num_multi_escape*N)',1,size(I_lim,2))
    WBF_FER_sim = wbf_bler_multi_escape_p./repmat((total_codeword_num_multi_escape)',1,size(I_lim,2)) % FER is aka BLER

for theta=0:-0.1:-1.5
    ...
    % if exist("wbf_ber_multi","var") ~=1
    
    wbf_ber_multi= zeros(size(EbN0dB,2),size(I_lim,2));
    wbf_ber_multi(:,:)=-1;
    wbf_bler_multi= zeros(size(EbN0dB,2),size(I_lim,2));
    wbf_bler_multi(:,:)=-1;
    total_codeword_num_multi = zeros(size(EbN0dB));
    % end
    ...
    for idx=1:length(EbN0dB)
        while any(wbf_bler_multi(idx,:) < minimum_codeword_error_num)
            total_codeword_num_multi(idx) = total_codeword_num_multi(idx) + 1;
            % r=wbf_codeword_error_num(idx,:)
            % K-bit source data genera  tion
                ...
                if sum(Syn_wbf)>0
                    ...
                    % reach some limit
                        ...
                        if wbf_Num_error_bit_temp > 0
                            wbf_ber_multi(idx,col) = wbf_ber_multi(idx,col) + wbf_Num_error_bit_temp;
                            wbf_bler_multi(idx,col) = wbf_bler_multi(idx,col) + 1
                        end
                    end
                else
    ...
    WBF_BER_sim = wbf_ber_multi./repmat((total_codeword_num_multi*N)',1,size(I_lim,2))
    WBF_FER_sim = wbf_bler_multi./repmat((total_codeword_num_multi)',1,size(I_lim,2)) % FER is aka BLER
    ...
    hold off;
    legend;
    title(['Error-Rate Performance of (' num2str(N) ',' num2str(K) ') GDBF Multiple Flip Code; AWGN Channel']);
    xlabel('Eb/N0 (dB)');
    ylabel('Error Rates');
end
